...
\subsection{The Mathematical Model}
 ...
 \end{equation*}
 This last equation can be substituted by different models i.e. Euler Angles kinematic equations that can be more suitable. A simplified model have been derived considering system simmetry. Force components on y and z axes arise due to TM rotation. Angular velocity is expected to be of the order of few $\mu rad/s$, the contact between TM and RTs lasts much less than $1 ms$, as a result rotation angle is a fraction of $nrad$ and is neglected. Rotation arises for a modeled misalignment on y-axis only. So the linear dynamics as well as the rotational ones occur on one axis only. Therefore System is widely symplified:
 \begin{align}
 &M\ddot{x_T}=F\\ 	
 &I\dot{\omega}=M\\ 
 &\dot{\theta}=\omega
